...
public class CompilerTest {
    ...
    public CompilerTest(String expectedCompilerMessageId) throws URISyntaxException, IOException {
        try (DirectoryStream<Path> paths = Files.newDirectoryStream(
                Paths.get(DefaultCompiler.class.getResource("test").toURI()),
                path -> path.getFileName().toString().startsWith("test_" + expectedCompilerMessageId))) {
            this.rootSourcePaths = StreamSupport.stream(paths.spliterator(), false).toArray(Path[]::new);
        }

        this.expectedCompilerMessageId = expectedCompilerMessageId;
    ...
    public static Collection<Object[]> parameters() {
        return Arrays.stream(CompilerMessageTemplates.class.getFields())
                ...
                .map(compilerMessageTemplate -> new Object[]{compilerMessageTemplate.getId()})
    ...
    public void before() {
        ...
        compiler = new DefaultCompiler(LOGGER, cacheDirectory);
    }
